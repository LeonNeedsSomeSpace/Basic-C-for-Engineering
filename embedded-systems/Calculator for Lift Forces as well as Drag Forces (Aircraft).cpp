...
class Aircraft {
	...
	//The aircraft in question is rather compact
private: 
	double AirDensity; 
	double Velocity; 
	double WingArea; 
	...
	//Avoiod printing LiftForce and DragForce before calculating them
...
public: Aircraft(double _AirDensity, double _Velocity, double _WingArea, double _LiftCoefficient, double _DragCoefficient) : AirDensity(_AirDensity), Velocity(_Velocity), WingArea(_WingArea), LiftCoefficient(_LiftCoefficient), DragCoefficient(_DragCoefficient), LiftForce(0), DragForce(0) { 
	...
	//By wrapping up the values for the variables in these functions, it improves flexibility such as changes in the future when actual sensors are used to retrieve data
private: 
